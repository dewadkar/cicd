    ...
    function(err, decryptedJenkinsPassword) {
      ...
      request.post({
        ...
        if (error) {
          ...
          res.redirect("/automate/pattern15?error=true");
        } else {          
...
function createJob(req,res){
  ...
  console.log("Setting up pattern 15");  
  var pathToJson = process.env.TOOLSTITCH_HOME + '/teams/' + req.session.team + '.json';
  var path = require('path');
  ...
  try {
    ...
    prop.artifactoryhost = teamConfig.pipeline[from].config.artifactory.serveruri;
  ...
  if (isConfigValidated) {
    ...
    async.waterfall([function(callback) {
      var selectedTools = ['slack', 'bluemix', 'sonar', 'github', 'maven', 'artifactory'];
      var fs = require("fs");
